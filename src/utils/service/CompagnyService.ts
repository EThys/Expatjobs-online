...
export const useCompanyService = () => {
  ...
  const getCompaniesByUser = async (userId: number): Promise<ICompany[]> => {
    try {
      ...
      console.error(
        `❌ Erreur lors de la récupération des entreprises de l'utilisateur ${userId}:`,
        error,
      )
      throw error
    }
  ...
  const getAllCompanies = async (
    ...
    try {
      const response = await useAxiosRequestWithToken().get(`${ApiRoutes.getAllCompagny}`, {
        params: {
          page,
          size,
          sort,
  ...
  const updateCompany = async (
    id: number,
    companyData: Partial<ICompanyCreate>,
  ): Promise<ICompany> => {
    try {
  ...
  const deleteCompany = async (id: number): Promise<void> => {
    try {
      await useAxiosRequestWithToken(token).delete(`${ApiRoutes.getAllCompagny}/${id}`)
    } catch (error) {
