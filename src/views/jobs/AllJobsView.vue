    ...
    <transition name="page-fade" appear>
      <div class="flex flex-col flex-1" key="all-jobs-page">
        ...
        <main class="flex-grow">
          <PageHero>
            <template #title>
              {{ $t('hero.allJobsView.title').split($t('hero.allJobsView.titleHighlight'))[0] }}
              <span class="text-emerald-400">{{ $t('hero.allJobsView.titleHighlight') }}</span>
            </template>
            <template #subtitle>
            ...
            <div class="max-w-3xl mx-auto">
              <div
                class="relative bg-white/10 backdrop-blur-md rounded-2xl p-2 border border-white/20 shadow-2xl"
              >
                <div class="flex flex-col sm:flex-row gap-2">
                  <div class="flex-1 relative group">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-5 w-5 text-emerald-200 absolute left-4 top-4 transition-colors group-focus-within:text-emerald-400"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
          ...
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4 mb-6 border border-gray-200/80">
              <div class="flex items-center justify-between mb-4">
                <h3 class="text-base font-semibold text-gray-800">Filtres de recherche</h3>
                <button
                  @click="resetFilters"
              ...
              <SearchFilters :filters="filters" @filter="applyFilters" />
            ...
            <div class="space-y-8">
              <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-200/80">
                ...
                >
                  <div>
                    <h2 class="text-2xl font-bold text-gray-800">Offres d'emploi</h2>
                    <p class="text-gray-600 mt-2 flex items-center gap-2">
                      <span
                        >{{ totalElements }} offre{{ totalElements !== 1 ? 's' : '' }} disponible{{
                          totalElements !== 1 ? 's' : ''
                        }}</span
                      >
                ...
                <div v-if="loading" class="space-y-6">
                  <div
                    v-for="n in 6"
                    :key="n"
                    class="bg-white rounded-2xl border border-gray-200/80 p-6 shimmer-container"
                  >
                    <div class="flex items-start gap-4">
                      <div class="w-16 h-16 bg-gray-200 rounded-2xl shimmer"></div>
                      <div class="flex-1 space-y-3">
                        <div
                          class="h-6 bg-gray-200 rounded shimmer"
                          :style="{ width: Math.random() * 40 + 60 + '%' }"
                        ></div>
                        <div
                          class="h-4 bg-gray-200 rounded shimmer"
                        ...
                        <div class="flex gap-2 mt-4">
                          <div
                            class="h-6 bg-gray-200 rounded-full shimmer"
                            style="width: 60px"
                          ></div>
                          <div
                            class="h-6 bg-gray-200 rounded-full shimmer"
                ...
                <div v-else class="bg-white rounded-2xl border border-gray-200/80 p-12 text-center">
                  <div class="max-w-md mx-auto">
                    <div
                      class="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-emerald-100 to-teal-100 rounded-3xl flex items-center justify-center"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                    ...
                    <p class="text-gray-500 mb-6 leading-relaxed">
                      Aucune offre ne correspond à vos critères actuels. Essayez d'élargir votre
                      recherche ou de modifier vos filtres.
                    </p>
...
const currentPage = computed({
  get: () => storePage.value,
  set: (val) => {
    /* triggered by Pagination event, handled in changePage */
  },
})
...
const initFromQuery = async (isInitial = false) => {
  ...
  const hasAnyQuery = q.trim() !== '' || location.trim() !== '' || filters.value.typeContrat !== ''
