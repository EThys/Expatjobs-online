<template>
  <div class="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 overflow-x-hidden">
    ...
    <main class="flex-grow pt-24 pb-20 relative">
      ...
      <div class="absolute top-0 left-0 w-full h-[600px] bg-gradient-to-br from-emerald-500/5 to-teal-500/5 -skew-y-3 z-0 pointer-events-none"></div>
      <div class="absolute -top-40 -right-40 w-[600px] h-[600px] bg-blue-400/10 rounded-full blur-[120px] pointer-events-none"></div>
      <div class="absolute top-40 -left-20 w-[400px] h-[400px] bg-emerald-400/10 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div class="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="max-w-4xl mx-auto text-center mb-16 animate-fade-in-up">
          <h1 class="text-4xl md:text-6xl font-black tracking-tight text-slate-900 mb-6 leading-tight">
            TROUVEZ VOTRE <br />
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-600">PROCHAINE SCÈNE</span>
          </h1>
          ...
          <!-- Search Bar -->
          <div class="bg-white p-2 rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-100 flex flex-col md:flex-row gap-2 max-w-3xl mx-auto transition-all hover:shadow-[0_30px_80px_-20px_rgba(16,185,129,0.15)] ring-1 ring-gray-100 focus-within:ring-2 focus-within:ring-emerald-500/50">
          ...
          <div class="flex justify-center md:flex-wrap gap-3 min-w-max md:min-w-0 px-4">
             <button 
                ...
                :class="[
                  ...
                  !selectedCategory ? 'bg-slate-900 text-white border-slate-900 shadow-md transform scale-105' : 'bg-white text-slate-500 border-gray-200 hover:border-emerald-200 hover:text-emerald-600'
                ]"
             >
               TOUT
             </button>
             ...
             >
               <component :is="cat.icon" class="w-4 h-4" :class="selectedCategory === cat.id ? 'text-white' : 'text-slate-400'" />
        ...
        <!-- Results Info -->
        <div class="flex items-center justify-between mb-8 px-2 animate-fade-in" v-if="!loading || paginationLoading">
          <h2 class="text-xl font-bold text-slate-800">
            <span v-if="loading">Recherche en cours...</span>
            <span v-else>
              {{ totalElements }} offre{{ totalElements > 1 ? 's' : '' }} <span class="text-slate-400 font-normal">disponible{{ totalElements > 1 ? 's' : '' }}</span>
            </span>
          </h2>
        </div>

        <!-- Jobs Grid -->
        <div v-if="loading && !paginationLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           <div v-for="i in 9" :key="i" class="h-64 bg-white rounded-3xl border border-gray-100 shadow-sm animate-pulse"></div>
        </div>

        <div v-else-if="displayedJobs.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in-up delay-100">
           <div 
           ...
           >
             <!-- Accent line -->
             <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-gray-100 via-emerald-400 to-gray-100 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></div>

             <div class="flex justify-between items-start mb-6">
                <div class="w-14 h-14 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center p-2 group-hover:bg-white transition-colors">
                  <img :src="job.logo" :alt="job.companyData?.name || job.company?.name || ''" class="w-full h-full object-contain mix-blend-multiply" @error="handleImageError($event, job.companyData)" />
                </div>
                <span class="px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  {{ job.typeLabel }}
                </span>
             </div>

             <div class="mb-4">
               <h3 class="text-lg font-bold text-slate-900 leading-snug mb-1 group-hover:text-emerald-600 transition-colors line-clamp-2">
                 {{ job.title }}
               </h3>
             ...
             <div class="flex flex-wrap gap-2 mb-6 mt-auto pt-4">
                <span class="inline-flex items-center text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded-md">
                   <MapPinIcon class="w-3 h-3 mr-1 text-emerald-500" />
                   {{ job.location }}
                </span>
                <span v-if="job.salary" class="inline-flex items-center text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded-md">
                   <CurrencyEuroIcon class="w-3 h-3 mr-1 text-emerald-500" />
                   {{ job.salary }}
                </span>
             </div>

             <div class="mt-4 pt-4 border-t border-gray-50 flex justify-between items-center group/btn">
                <span class="text-xs font-medium text-slate-400">Publié le {{ formatDate(job.createdAt) }}</span>
                <router-link :to="`/detail/jobs/${job.id}`" class="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center group-hover/btn:bg-emerald-500 transition-colors">
...
const jobCategories = [
  ...
  { id: 'business', name: 'Business', icon: BriefcaseIcon, sectors: ['SALES', 'CONSULTING', 'BUSINESS'] },
  { id: 'finance', name: 'Finance', icon: BanknotesIcon, sectors: ['FINANCE', 'ACCOUNTING'] },
  { id: 'marketing', name: 'Marketing', icon: GlobeAltIcon, sectors: ['MARKETING', 'COMMUNICATION'] },
...
const getCompanyLogo = (company?: ICompany): string => {
  ...
  if (company?.webSiteUrl) {
    ...
    } catch { return getFallbackLogo(company) }
  }
...
const formatJobForDisplay = (job: IJobOffers) => {
  return {
    ...
    typeLabel: job.jobType === 'FULL_TIME' ? 'CDI' : (job.jobType === 'CONTRACT' ? 'CDD' : job.jobType),
