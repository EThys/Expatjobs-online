...
const login = async () => {
  ...
  try {
    const response = await useAxiosRequestWithToken().post(`${ApiRoutes.login}`, data, {
      signal: abortSignal,
    })
...
<template>
  <div class="min-h-screen w-full flex items-stretch overflow-hidden bg-white">
    ...
    <div
      class="w-full lg:w-1/2 xl:w-5/12 flex flex-col justify-center items-center p-6 sm:p-12 relative z-10 bg-white/80 backdrop-blur-md"
    >
      <!-- Logo & Home Link -->
      ...
      <div class="w-full max-w-md mx-auto animate-fade-in-up">
        ...
        <div class="text-center mb-10">
          <div
            class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/30 mb-6 transform hover:rotate-6 transition-transform duration-300"
          >
            <svg
        ...
        <form @submit.prevent="loginValidate" class="space-y-6">
          ...
          <div class="flex items-center justify-between text-sm">
            <label class="flex items-center group cursor-pointer">
              ...
              />
              <span class="ml-2 text-gray-500 group-hover:text-gray-700 transition-colors">{{
                t('auth.login.rememberMe')
              }}</span>
            </label>
            <a
              href="#"
              class="font-medium text-emerald-600 hover:text-emerald-700 hover:underline transition-all"
      ...
      <div class="absolute bottom-6 text-center w-full">
        <p class="text-xs text-gray-400">
          &copy; {{ new Date().getFullYear() }} EXPAT JOBS Go. All rights reserved.
        </p>
      </div>
    </div>
    ...
    <div class="hidden lg:block lg:w-1/2 xl:w-7/12 relative bg-gray-900 overflow-hidden">
      <!-- Background Image -->
      <div
        class="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1522071820081-009f0129c71c?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80')] bg-cover bg-center opacity-60 mix-blend-overlay hover:scale-105 transition-transform duration-[20s]"
      ></div>
      ...
      <!-- Blobs -->
      <div
        class="absolute top-1/4 left-1/4 w-96 h-96 bg-emerald-500/30 rounded-full mix-blend-screen filter blur-[100px] animate-blob"
      ></div>
      <div
        class="absolute bottom-1/4 right-1/4 w-96 h-96 bg-cyan-500/30 rounded-full mix-blend-screen filter blur-[100px] animate-blob animation-delay-2000"
      ></div>
        ...
        >
          <h2 class="text-4xl font-black text-white mb-6 leading-tight">
            ...
            <span
              ...
              >{{ t('auth.login.sideHighlight') }}</span
            >
          </h2>
          ...
          <div class="grid grid-cols-3 gap-6 pt-6 border-t border-white/10">
            <div>
              ...
              <span class="text-xs text-emerald-200 uppercase tracking-wider">{{
                t('auth.login.stats.offers')
              }}</span>
            </div>
            <div>
              <span class="block text-2xl font-bold text-white">50+</span>
              <span class="text-xs text-emerald-200 uppercase tracking-wider">{{
                t('auth.login.stats.countries')
              }}</span>
            </div>
            <div>
              <span class="block text-2xl font-bold text-white">100%</span>
              <span class="text-xs text-emerald-200 uppercase tracking-wider">{{
                t('auth.login.stats.verified')
              }}</span>
            </div>
          </div>
        </div>
